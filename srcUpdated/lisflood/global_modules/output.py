...
import os
import numpy as np
from pcraster import ifthen, catchmenttotal, mapmaximum, report
import sys
...
class PCRasterWriter(Writer):
    ...
    def __init__(self, var, map_path, map_value, map_key):          # map_value and parameter was added
        self.var = var
        self.map_key = map_key
        #self.map_path = map_path
        self.settings = LisSettings.instance()
        ...
        if self.settings.mc_set:
            # MonteCarlo model
            base_dir = "/kaiganY/kumudu/LF_ModelRun/LF_ETRS89/NEW_APPROACH/Event_1/6/A"
            sample_number = self.var.currentSampleNumber()
            self.map_path = os.path.join(base_dir,str(sample_number), binding[self.map_key].split("/")[-1])
        else:
            self.map_path = map_path
...
class MapOutput():
    ...
    def __init__(self, var, out_type, frequency, map_key, map_value):
        ...
        if self.is_valid():
            if option['writeNetcdf'] or option['writeNetcdfStack']:
                ...
                self.writer = PCRasterWriter(self.var, self.map_path, self.map_value, self.map_key)
    ...
    def _extract_path(self):
        ...
        if self.settings.mc_set:
            # MonteCarlo model
            sample_number = self.var.currentSampleNumber()
            self.map_path = os.path.join(str(sample_number), binding[self.map_key].split("/")[-1])
        else:
            self.map_path = binding.get(self.map_key)
