...
from .settings import datetoint, LisSettings, MaskInfo, MaskAreaInfo,inttodate,calendar
    ...
    def run(self):
        ...
        if hasattr(self._userModel(), "resume"):
            ...
            if self._userModel().firstTimeStep() == datetoint(binding['StepStart'])[0]:
                self._runInitial()
                print("This model has resume method, but starting from the begining.")
            else:
                self._runResume()
                print("This model has resume method,and using saved state varibles")
        else:
            self._runInitial()
            print('This model does not have resume method, starting from the beginning')
        ...
        if hasattr(self._userModel(), "suspend") and hasattr(self._userModel(), "filterPeriod"):
            self._runSuspend()
            print('Running the filter framework')
...
class EnsKalmanFilterFramework(EnsKalmanFilterFramework):
      ...
      if(hasattr(self._userModel(), 'run')):
        ...
        for currentPeriod in range(0, len(self._userModel()._d_filterTimesteps) + 1):
          ...
          if not currentPeriod == lastPeriod:
            ...
            # ── CONDITIONAL PARENT RESUME ──
            mc = self._d_model
            if hasattr(mc, '_forkSamples') and mc._forkSamples():
                # only when fork‑samples is on
                self._runResume()
    ...
    def _startEndOfPeriod(self, currentPeriod, lastPeriod):
        ...
        # ——— DEBUG LOGGING ———
        # Pull global StepStart/StepEnd from your settings
        settings     = LisSettings.instance()
        binding      = settings.binding
        global_start = binding.get('StepStart')
        global_end   = binding.get('StepEnd')
        
        ref_date = calendar(binding['CalendarDayStart'] ,binding['calendar_type'])
        
        start_date = inttodate(startTimestep - 1, ref_date, binding=binding)
        end_date   = inttodate(endTimestep - 1, ref_date, binding=binding)
        # Print them plus the computed period boundaries
        print(f"[DEBUG] Global StepStart={global_start}, StepEnd={global_end}")
        print(f"[DEBUG] Assimilation period {currentPeriod}/{lastPeriod}: "
              f"startTimestep={startTimestep}, endTimestep={endTimestep}")
        print(f"→ dates {start_date} to {end_date}")
        
        # now convert to calendar dates:
