...
from .global_modules.add1 import loadsetclone, mapattrNetCDF, perturbState
...
class LisfloodModel_ini(DynamicModel):
    ...
    def __init__(self):
        ...
        # ------------------------------------------------------------------
        # Accessing the forcings dictionary
        #self.precipitation_forcings = self.readmeteo_module.forcings['PrecipitationMaps']
        
        #scaling_factor =  np.random.lognormal(mean=1, sigma=0.20, size=self.precipitation_forcings.shape)
        #scaled_precip = self.precipitation_forcings * scaling_factor
        #self.readmeteo_module.forcings['PrecipitationMaps'] = scaled_precip
        #print('PrecipitationMaps are perturbed')
        # -----------------------------------------------------------------
...
# ====== INITIAL ================================
    def initial(self):
        ...
        """
        np.random.seed(120+self.currentSampleNumber())
        print('Current Sample Number:', self.currentSampleNumber())
        ...
        # Perturbe the PrScaling factor
        #self.readmeteo_module.var.PrScaling = round(np.random.uniform(0.8,1.50),2)
        self.readmeteo_module.var.PrScaling = 1.50
        print('PrScaling Factor:', self.readmeteo_module.var.PrScaling)
        self.readmeteo_module.var.prev_gauss = None
        
        # Perturbe the InfScaling factor
        self.inflow_module.var.InfScaling = round(np.random.uniform(0.75,1.25),2)
        print('InfScaling Factor:', self.inflow_module.var.InfScaling)
        
        #self.UZScaling = round(np.random.uniform(0.5,2.0),2)
        #self.groundwater_module.var.UpperZoneK = np.minimum((self.UZScaling * self.groundwater_module.var.UpperZoneK ), 1)
        #print('UZScaling:', self.UZScaling)
