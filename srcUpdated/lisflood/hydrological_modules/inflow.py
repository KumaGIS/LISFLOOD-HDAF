...
# --------------------------------------------------------------------------
    ...
    def dynamic(self):
        ...
        if option['inflow']:
            ...
            # This part is updated by Madhawa (Testing)
            # Apply scaling factor
            self.var.QIn_scaled = QIn * self.var.InfScaling
            
            # Apply Gaussian white noise perturbation
            noise_std = QIn * 0.10
            noise = np.random.normal(loc=0, scale=noise_std, size= self.var.QIn_scaled.shape)
            QIn_perturbed = np.maximum(self.var.QIn_scaled + noise,0)                           # make sure the perturbed inflow data does not below 0
            
            #self.var.QInM3 = QIn * self.var.DtSec
            self.var.QInM3 = QIn_perturbed * self.var.DtSec                                     # testing
